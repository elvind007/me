...
import './GlobalFileSharing.css';

const ipfs = IPFS.create({ host: 'ipfs.infura.io', port: 5001, protocol: 'https' });

const GlobalFileSharing = ({ web3, accounts, globalFileSharingContract }) => {
  ...
  const addFileToIPFS = async () => {
    try {
      ...
      // For example: await globalFileSharingContract.methods.storeFileHash(result.path).send({ from: accounts[0] });
    } catch (error) {
  ...
  const getFileFromIPFS = async () => {
    try {
      ...
      // For example: const storedFileHash = await globalFileSharingContract.methods.getFileHash().call();
...
export default GlobalFileSharing;